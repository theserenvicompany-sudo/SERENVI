/**
 * replace-products.js
 * Deletes the previously seeded Men's batch (273 products added on Jul 28)
 * then re-seeds fresh from the scraped list with 7× pricing
 */
...
const PRODUCTS = [
  ...
  ['Graphic Print Loose Fit Crew-Neck T-Shirt (Black)', 150, 'https://assets.ajio.com/medias/sys_master/root/20230804/beHF/64ccd2d7a9b42d15c98b97e6/eyebogler_black_graphic_print_loose_fit_crew-neck_t-shirt.jpg'],
  ...
  ['Graphic Oversized Fit T-shirt (White)', 325, 'https://assets.ajio.com/medias/sys_master/root1/20260506/CntI/69fb0c04fcb5bb61d2a13e2e/red_tape_white_graphic_oversized_fit_t-shirt.jpg'],
  ...
  // No-image products (274 onwards from scraper + remaining from original list)
  ['Men Regular Fit Cotton Half-Sleeve Crew-Neck T-Shirts (Beige)', 14, null],
  ...
  ['Men Checked Regular Fit Shirt (White)', 200, null],
  ['Men Checked Slim Fit Shirt with Spread Collar (Green)', 250, null],
  ['Colourblock Half-Zip T-Shirt', 150, null],
  ['Short-Sleeve Polo T-Shirt (Navy)', 200, null],
  ['Football Athletic Socks (Red)', 76, null],
  ['Men Round-Toe Lace-Up Shoes (Olive)', 495, null],
  ['Men Regular Fit Cotton Half-Sleeve Crew-Neck T-Shirts (Turquoise)', 14, null],
  ...
  ['Men Regular Fit Cotton Half-Sleeve Crew-Neck T-Shirts (Ecru)', 14, null],
  ['Men Regular Fit High-Neck T-Shirt (Black)', 175, null],
  ['Men Regular Fit Shirt with Patch Pocket (Teal)', 250, null],
  ['Men Crew-Neck Oversized Fit T-Shirt', 220, null],
  ['Men Graphic Print Regular Fit T-shirt (Olive)', 175, null],
  ...
  ['Graphic Print Loose Fit Crew-Neck T-Shirt (Black 2)', 175, null],
  ['Men Striped Regular Fit Crew-Neck T-Shirt (Wine)', 200, null],
  ...
  ['Men Silver-Plated Chain with Clasp-Closure (2)', 130, null],
  ['Men Regular Fit Cotton Half-Sleeve Crew-Neck T-Shirts (Rust)', 14, null],
  ...
  ['Men Set of 2 Stone-Studded Rakhis', 200, null],
...
async function main() {
  console.log('🗑️  Deleting old seeded Men\'s products batch...');

  // Delete products created after July 28 2026 (our last seed batch)
  const deleted = await prisma.product.deleteMany({
    where: {
      createdAt: { gte: new Date('2026-07-28T00:00:00Z') }
    }
  });
  console.log(`✅ Deleted ${deleted.count} old products`);

  console.log(`\n📦 Seeding ${PRODUCTS.length} fresh products (7× pricing)...\n`);
