...
function findHighestTier(monthlySales) {
  ...
  return -1; // No tier qualification
}

async function updateLeadershipSalary() {
  ...
  console.log('\n💰 LEADERSHIP SALARY RECALCULATION');
  console.log('===================================\n');
  ...
  // Get total referred revenue for the pool
  ...
  const pool = totalReferredRevenue.mul(new Decimal(0.15)); // 15% pool
  const newSalary = pool.mul(new Decimal(tier.percentage)).div(new Decimal(100));

  console.log(`📊 SALARY CALCULATION:`);
  console.log(`   Total Referred Revenue: ₹${totalReferredRevenue}`);
  console.log(`   15% Pool: ₹${pool}`);
  console.log(`   ${tier.percentage}% of Pool: ₹${newSalary}\n`);

  if (user.currentLeadershipRank === tierIndex) {
    console.log(`ℹ️  Same tier - only amount might have changed`);
  } else if (user.currentLeadershipRank !== null && user.currentLeadershipRank !== tierIndex) {
    console.log(`⬆️  TIER UPGRADE: ${SALARY_TIERS[user.currentLeadershipRank].name} → ${tier.name}`);
  } else {
...
updateLeadershipSalary().catch(console.error);
